...
local intern_version = "5.0 WotLK - Beta";
...
local function SwitchRaidProfiles()
    if not InCombatLockdown() or changeableValues_DB.ChangesInCombat then
        ...
        if HasLoadedCUFProfiles() then
            if not InCombatLockdown() and savedValues_DB.Profile ~= nil then
...
local function CheckProfileOptions()
    if GetNumGroupMembers() <= 5 and GetActiveRaidProfile() == savedValues_DB.Profile then
        if not CompactUnitFrameProfilesRaidStylePartyFrames:GetChecked() then
...
local function ApplySort(SortOption, RaidProfileName, ExternSwitch)
    if ExternSwitch then
        SwitchRaidProfiles();
    elseif not InCombatLockdown() then
        if RaidProfileName ~= nil and GetNumGroupMembers() <= 5 and HasLoadedCUFProfiles() then
            if GetActiveRaidProfile() ~= RaidProfileName and not AutoActivate and savedValues_DB.AlwaysActive == false then
                if internValues_DB.showChatMessages then
                    if savedValues_DB.ChatMessagesOn then
                        print(ColorText(L["SortGroup_RaidProfil_Doesnt_match_output_output"], "option"));
                    end
                end
            elseif GetActiveRaidProfile() == RaidProfileName or savedValues_DB.AlwaysActive then
...
local function SortInterstation(ExternSwitch)
    if not savedValues_DB.AlwaysActive then
        if savedValues_DB.Top or savedValues_DB.Bottom then
            if not ProfileExists(savedValues_DB.Profile) then
                if savedValues_DB.Profile == nil then
                    savedValues_DB.Profile = "nil";
                end
                local cachePrintSendSortOptionToSortBy = L["SortGroup_RaidProfil_dont_exists_output"]:gsub(
                    "'replacement'", savedValues_DB.Profile);
                savedValues_DB.Profile = GetRaidProfileName(1);
                if savedValues_DB.ChatMessagesOn then
                    print(ColorText(cachePrintSendSortOptionToSortBy:gsub("'replacement2'", savedValues_DB.Profile),
                        "option"));
                end
    ...
    if savedValues_DB.Top then
        ApplySort('Down', savedValues_DB.Profile, false);
    elseif savedValues_DB.Bottom then
        ApplySort('Up', savedValues_DB.Profile, false);
    else
        if internValues_DB.showChatMessages then
            if savedValues_DB.ChatMessagesOn then
...
-- Breakpoint between Methods and ApplySort
...
local function ProfileChangedEvent()
    if changeableValues_DB.CatchProfileSwitchEvent then
        ...
        hooksecurefunc("CompactUnitFrameProfiles_ActivateRaidProfile", function(profile)
            if (not InCombatLockdown() or changeableValues_DB.ChangesInCombat) and savedValues_DB.AlwaysActive == true then
...
local function createText()
    ...
    Option_Text_Utility:SetFontObject('p', GameFontHighlightMedium);
...
local function frameEvent()
    ...
    Main_Frame:SetScript("OnEvent", function(self, event, ...)
        ...
        elseif event == "PLAYER_REGEN_ENABLED" then
            ...
            if savedValues_DB.AutoActivate or SortGroup_Variable_Page2_AdditionalSwitchActive then
                SortInterstation(true);
            else
                SortInterstation(false);
            end
